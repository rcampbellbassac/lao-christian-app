...
<template>
  <div v-if="audio || video" class="mt-5 flex flex-wrap gap-2 rounded-xl border border-[var(--lc-border)] bg-[var(--lc-soft)] p-3">
    <a v-if="audio" :href="audio" target="_blank" rel="noopener noreferrer" class="app-chip">♫ {{ copy.text('media.audio') }}</a>
    <a v-if="video" :href="video" target="_blank" rel="noopener noreferrer" class="app-chip">▶ {{ copy.text('media.video') }}</a>
  </div>
</template>
