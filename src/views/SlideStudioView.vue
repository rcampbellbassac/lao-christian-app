...
const deck = computed(() =>
  route.params.deckId ? decks.getDeck(route.params.deckId as string) : undefined,
)
const selectedIndex = computed(
  () => deck.value?.slides.findIndex((slide) => slide.id === selectedSlideId.value) ?? -1,
)
const selectedSlide = computed(() =>
  selectedIndex.value >= 0 ? deck.value?.slides[selectedIndex.value] : undefined,
)
const exportStageStyle = computed(() => {
  const preset = getAspectRatioPreset(deck.value?.aspectRatio ?? '16:9')
  return {
...
function changeDeckFont(step: number): void {
  ...
  deck.value.fontScale = Math.min(
    FONT_SCALE_MAX,
    Math.max(FONT_SCALE_MIN, Math.round((deck.value.fontScale + step) * 10) / 10),
  )
  void saveDeck()
}
...
async function toggleSelectedSlideVisibility(): Promise<void> {
  if (!selectedSlide.value) return
  selectedSlide.value.hidden = !selectedSlide.value.hidden
  await saveDeck()
}
...
async function runExport(kind: 'png' | 'zip' | 'pptx'): Promise<void> {
  ...
  if (!visibleSlides.length) {
    status.value = copy.text('studio.noVisible')
    return
  }
  ...
  try {
    ...
    if (kind === 'png') {
      const slide =
        selectedSlide.value && !selectedSlide.value.hidden ? selectedSlide.value : visibleSlides[0]!
      await exporter.exportSlideAsPng(
        { ...slide, element: await renderForExport(slide) },
        preset,
        deck.value.name,
      )
    } else if (kind === 'zip') {
  ...
  <main class="app-page">
    <section v-if="!deck" class="app-panel">
      ...
      <div class="flex items-center justify-between gap-3">
        <div>
          <h1 class="app-section-title">ສະຕູດິໂອສະໄລ້</h1>
          <p class="app-muted"><BilingualText text-key="studio.localHelp" /></p>
        </div>
        <button type="button" class="studio-primary" @click="newDeck">
          ＋ {{ copy.text('studio.newDeck') }}
        </button>
      </div>
      <ul class="mt-6 grid gap-3 sm:grid-cols-2">
        <li v-for="item in decks.decks" :key="item.id" class="lc-card p-4">
          <router-link :to="`/decks/${item.id}`" class="app-link text-lg">{{
            item.name
          }}</router-link>
          <p class="app-muted mt-1 text-sm">
            {{ item.slides.length }} {{ copy.text('studio.slides') }} ·
            {{ new Date(item.updatedAt).toLocaleDateString() }}
          </p>
        </li>
      </ul>
      <p v-if="decks.isLoaded && !decks.decks.length" class="app-muted py-12 text-center">
        <BilingualText text-key="studio.empty" />
      </p>
    </section>
    ...
    <section v-else class="studio-shell">
      ...
      <header class="studio-toolbar">
        ...
        <input
          v-model="deck.name"
          class="studio-title"
          :aria-label="copy.text('studio.deckName')"
          @change="saveDeck"
        />
        <span class="app-muted text-xs">{{ status }}</span>
        <router-link :to="`/present/deck/${deck.id}`" class="studio-primary"
          >▶ {{ copy.text('studio.present') }}</router-link
        >
        <button
          type="button"
          class="app-chip"
          :disabled="isExporting"
          :title="copy.text('presentation.exportPng')"
          @click="runExport('png')"
        >
          PNG
        </button>
        ...
        <button
          type="button"
          class="app-chip"
          :title="copy.text('studio.deleteDeck')"
          :aria-label="copy.text('studio.deleteDeck')"
        ...
        <aside class="studio-sidebar">
          ...
          <ol class="studio-slide-list mt-3 grid gap-2">
            <li
              ...
              :key="slide.id"
              class="studio-slide-item"
              :class="{ 'studio-slide-item--active': slide.id === selectedSlideId }"
              @click="selectedSlideId = slide.id"
            >
              <span class="studio-slide-number">{{ index + 1 }}</span
              ><span class="studio-slide-name" :title="slide.title">{{ slide.title }}</span
              ><span v-if="slide.hidden">◌</span>
            </li>
          </ol>
        </aside>
        <section v-if="selectedSlide" class="studio-editor">
          <div class="studio-slide-preview">
            <DeckSlideCanvas
              :slide="selectedSlide"
          ...
          <label
            >{{ copy.text('studio.slideTitle')
            }}<input v-model="selectedSlide.title" class="studio-input" @change="saveDeck"
          /></label>
          <label
            >{{ copy.text('studio.slideText')
            }}<textarea
              :value="textFromHtml(selectedSlide.html)"
              rows="7"
              class="studio-input"
              @change="updateSlideBody(selectedSlide, $event)"
            ></textarea>
          </label>
          <label
            >{{ copy.text('studio.speakerNotes')
            }}<textarea
              v-model="selectedSlide.speakerNotes"
              rows="4"
              class="studio-input"
              @change="saveDeck"
            ></textarea>
          ...
          <div class="flex flex-wrap gap-2">
            <button
              type="button"
              class="app-chip"
              :title="copy.text('studio.moveUp')"
              :aria-label="copy.text('studio.moveUp')"
              @click="decks.moveSlide(deck, selectedIndex, -1)"
            >
              ↑
            </button>
            <button
              type="button"
              class="app-chip"
              :title="copy.text('studio.moveDown')"
              :aria-label="copy.text('studio.moveDown')"
              @click="decks.moveSlide(deck, selectedIndex, 1)"
            >
              ↓
            </button>
            <button
              type="button"
              class="app-chip"
              :title="copy.text('studio.duplicate')"
              :aria-label="copy.text('studio.duplicate')"
              @click="decks.duplicateSlide(deck, selectedIndex)"
            >
              ⧉
            </button>
            <button
              type="button"
              class="app-chip"
              :title="selectedSlide.hidden ? copy.text('studio.show') : copy.text('studio.hide')"
              :aria-label="
                selectedSlide.hidden ? copy.text('studio.show') : copy.text('studio.hide')
              "
              @click="toggleSelectedSlideVisibility"
            >
            ...
            <button
              type="button"
              class="app-chip"
              :title="copy.text('studio.deleteSlide')"
              :aria-label="copy.text('studio.deleteSlide')"
          ...
          <label
            >{{ copy.text('studio.aspectRatio')
            }}<select v-model="deck.aspectRatio" class="studio-input" @change="saveDeck">
          ...
          >
          <label
            >{{ copy.text('studio.theme')
            }}<PresentationThemePicker
              v-model="deck.theme"
              :aspect-ratio="deck.aspectRatio"
              @update:model-value="saveDeck"
          /></label>
          <div class="studio-format-grid">
            <label
              >{{ copy.text('presentation.font')
              }}<select v-model="deck.fontFamily" class="studio-input" @change="saveDeck">
            ...
            <div class="studio-control-group">
              ...
              <div class="flex items-center gap-2">
                <button
                  type="button"
                  class="app-chip"
                  :disabled="deck.fontScale <= FONT_SCALE_MIN"
                  @click="changeDeckFont(-FONT_SCALE_STEP)"
                >
                  −</button
                ><strong>{{ Math.round(deck.fontScale * 100) }}%</strong
                ><button
                  type="button"
                  class="app-chip"
                  :disabled="deck.fontScale >= FONT_SCALE_MAX"
                  @click="changeDeckFont(FONT_SCALE_STEP)"
                >
                  ＋
                </button>
            ...
            <label
              >{{ copy.text('presentation.textAlign')
              }}<select v-model="deck.textAlign" class="studio-input" @change="saveDeck">
                <option value="left">{{ copy.text('action.left') }}</option>
                <option value="center">{{ copy.text('action.center') }}</option>
              </select></label
            >
          </div>
        </section>
...
.studio-slide-preview {
  display: grid;
  max-height: 32rem;
  place-items: center;
  overflow: hidden;
  border-radius: 0.6rem;
  background: var(--lc-dark-bg);
}
.studio-slide-preview :deep(.deck-canvas) {
  width: min(100%, calc(32rem * var(--deck-ratio, 1.7778)));
  max-height: 32rem;
}
.export-stage {
  position: fixed;
