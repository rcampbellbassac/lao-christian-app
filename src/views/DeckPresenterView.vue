...
function broadcast(): void {
  const state: DeckPresentationState = {
    type: 'state',
    deckId,
...
<template>
  <main class="controller-shell">
    <header class="controller-toolbar">
      <router-link :to="`/decks/${deckId}`" class="controller-btn"
        >← {{ copy.text('presenter.edit') }}</router-link
      >
      <strong class="min-w-0 flex-1 truncate">{{ deck?.name }}</strong>
      <span>{{ currentIndex + 1 }} / {{ slides.length }}</span>
      <button type="button" class="controller-btn" @click="openAudience">
        ▣
        {{
          audienceOpen
            ? copy.text('presenter.reconnectAudience')
            : copy.text('presenter.openAudience')
        }}
      </button>
      <button type="button" class="controller-btn" @click="blank = !blank">
        {{ blank ? '◉' : '●' }}
        {{ blank ? copy.text('presenter.showSlide') : copy.text('presenter.blackScreen') }}
      </button>
    </header>
    ...
    <section v-if="deck && currentSlide" class="controller-grid">
      ...
      <aside class="controller-notes">
        <h2>{{ copy.text('studio.speakerNotes') }}</h2>
        <p class="whitespace-pre-wrap">
          {{ currentSlide.speakerNotes || copy.text('presenter.noNotes') }}
        </p>
        <h2 class="mt-6">{{ copy.text('presenter.next') }}</h2>
        <div v-if="nextSlide" class="next-card">
          <strong>{{ nextSlide.title }}</strong>
        </div>
        <p v-else class="app-muted">{{ copy.text('presenter.end') }}</p>
      </aside>
    </section>
    <footer class="controller-footer">
      <button type="button" class="controller-btn" :disabled="currentIndex === 0" @click="previous">
        ← {{ copy.text('presenter.previous') }}</button
      ><button
        type="button"
